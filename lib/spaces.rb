...
class Space
  ...
  def self.create(owner:, name:, description:, price:, from_date:, to_date:)
    ...
    Space.new(id: result[0]["id"], owner: result[0]["owner"], name: result[0]["name"], description: result[0]["description"], price: result[0]["price"], from_date: result[0]["from_date"], to_date: result[0]["to_date"])
  end

  def self.all
    result = DatabaseConnection.query('SELECT * FROM spaces;')
    result.map { |space| Space.new(id: space['id'], owner: space['owner'], name: space['name'], description: space['description'], price: space['price'], from_date: space['from_date'], to_date: space['to_date']) }
  end
